...
function SetServer(){
	...
	if( $domain <> '127.0.0.1' && $domain <> 'localhost' && C('config.web.cookie_type') == '1')
	{
		...
		{
			$cd = str_ireplace(TCP_TYPE.'://','',C('config.web.weburl'));
			//如果以/结尾了
			if( substr($cd, -1) == '/' )
			{
				$cd = substr($cd,0,-1);
...
/**
 * 获得毫秒数
 * @param 参数1，选填，是否补全为4为数字
 */
function GetMicroTime($iscomp=true)
{
	list($time,$mtime) = explode('.',microtime(true));
	if( $iscomp == true && strlen($mtime) < '4')
	{
		for($i=1;$i<=(4-strlen($mtime));$i++){
			$mtime .= '0';
		}
	}
	return $mtime;
			...
			if( $fieldOption )
			{
				foreach ($fieldOption as $k=>$v)
				{
					if( array_key_exists($k, $valueOption) )
					{
						...
						{
							$arr3[C('system.label.diy',null,'lang').':'.$v['title']] = implode(',', $valueOption[$k]);
							$arr3['field:'.$v['title']] = implode(',', $valueOption[$k]);
						}
						else
						{
							$arr3[C('system.label.diy',null,'lang').':'.$v['title']] = $valueOption[$k];
...
function CheckBasicConfig($isM=false)
{
	...
	if( $basicContent == '' || time()-intval($basicArr[0]) > 43200 )
	{
...
function FormCodeCheck($id = '', $isReturn = false)
{
	if( C('config.web.'.$id) == '1' )
	{
		...
		$sessionCode = Session('form_code');
		//验证码不通过
		if( $sessionCode == '' || !is_array($sessionCode) || !$sessionCode )
		{
			$lang = 'code_err';
		}
		//验证码通过
