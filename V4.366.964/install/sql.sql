...
CREATE TABLE `wm_system_msg_temp` (
  ...
  `temp_content` text NOT NULL COMMENT '发信内容',
  PRIMARY KEY (`temp_id`)
) ENGINE=MyISAM AUTO_INCREMENT=5 DEFAULT CHARSET=utf8 COMMENT='消息模版表';

/*Data for the table `wm_system_msg_temp` */

insert  into `wm_system_msg_temp`(`temp_id`,`temp_name`,`temp_module`,`temp_key`,`temp_content`) values (1,'禁止登录','user','user_punish_login','<p>您已经被系统禁止登录，处罚原因：{原因}。</p>'),(2,'禁止发言','user','user_punish_talk','<p>您已经被系统禁止发言，处罚原因：{原因}。</p>'),(3,'解除禁止登录','user','user_punish_unlogin','<p>您已经被系统解除禁止登录，请牢记平台规则，以免再次被封禁。</p>'),(4,'解除禁止发言','user','user_punish_untalk','<p>您已经被系统解除禁言，请牢记平台规则，以免再次被禁言。</p>');
