 ...
 */
function RepField($arr2 , $arr1 = array() , $value , $type = '1' , $label='', $filedData=array())
{
	if( !empty($arr1) )
	{
...
function FormCodeCreate($id)
{
	if( C('config.web.'.$id) == '1' )
	{
		switch(C('config.web.'.$id.'_type'))
		{
			case '1':
				$code = '';
				$html = '<img src="/wmcms/inc/imgcode.php?id='.$id.'" id="refresh" onClick="this.src=\'/wmcms/inc/imgcode.php?id='.$id.'&t=\'+Math.random()" class="imgcode" style="cursor: pointer;vertical-align: middle;display:-webkit-inline-box" />';
				break;
 ...
 * @return data
 */
function HiedField($data,$fileds)
{
	if( $data )
	{
