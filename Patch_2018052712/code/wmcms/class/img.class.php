...
class img {
     ...
     * 直接输出图片验证码。
     */
    static function ImgCode($len=4 , $size = 20 , $width = 0 , $height = 0){
    	//随机数
	...
	static function ImgCut($imgPath, $newWidth = '0', $newHeight = '0',$newPath = '')
	{
		...
		if( $newWidth == '0' || $newHeight == '0' )
		{
			...
			//上传图片达到多少宽度需要进行剪裁
			$newWidth = C('config.web.upload_imgwidth');
			$newHeight = C('config.web.upload_imgheight');

			//如果图片宽高任意一项低于了设置就不进行剪裁操作。
			if( $uploadCut != '1' || $imgWidth < $newWidth || $imgHeight < $newHeight )
			{
				return false;
			}
		}
		else
		{
			...
			//是否保存为新的图片
			if( trim($newPath) != '' ){
				imagepng($targetImage,$newPath);
			}else{
				imagepng($targetImage,$imgPath);
			}
    ...
    static function ToImg($str , $savePath = '', $imgWidth = 900, $size = '15' , $bg = array(0,0,0) , $fbg = array(255,255,255) )
    {
		...
		/* for ( $i=0 ; $i < str::StrLen($str,false) ; $i++ )
		{
			$letter[] = mb_substr($str, $i, 1 ,'utf-8');
		}
		foreach ( $letter as $l ) {
			$teststr = $newStr." ".$l;
			$testbox = imagettfbbox($size, $rot, $font, $teststr);
			// 判断拼接后的字符串是否超过预设的宽度
			if ( ( $testbox[2] > $imgWidth ) && ( $newStr !== "" ) ) {
				$newStr .= "\n";
			}
			$newStr .= $l;
		}
	...
	static function WaterMark($groundImage,$waterPos=9,$waterImage="",$waterText="",$textFont=10,$textColor="#000000")
	{
		...
		if( $waterImage == '' && $waterText == '' )
		{
			...
			if ( $waterMark == '1' && $imgWidth > $WMWidth  && $imgHeight > $WMHeight )
			{
				...
				{
					//水印图片地址
					$waterImg = WMROOT.'files/images/watermark.png';
				}
				else
				{
					//文字水印内容
