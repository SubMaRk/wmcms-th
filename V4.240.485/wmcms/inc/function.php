...
function GetUrl($post = false)
{
	...
	return TCP_TYPE.'://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].$par;
}
		...
		if( $v['field'] != '' )
		{
			...
			if( $fieldOption )
			{
				foreach ($fieldOption as $k=>$v)
				{
					...
					{
						$arr3[$label.C('system.label.diy',null,'lang').':'.$v['title']] = @$valueOption[$k];
						$arr3[$label.'field:'.$v['title']] = @$valueOption[$k];
					}
				}
			}
		}
