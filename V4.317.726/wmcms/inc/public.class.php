...
class common{
	...
	static function NativePhp($str='{php}[a]{/php}',$z=true)
	{
		...
		for ($nativePhpI = 0 ; $nativePhpI < count($phpArr[0]) ; $nativePhpI++ )
		{
			...
			$cacheTemp = WMCACHE.'temp/eval/'.date('Y-m').'/'.date('d').'-'.date('H').'-'.md5($phpContent.C('page.pagetype').C('page.path').C('page.tpath')).'.php';
				...
				if( count($symbolArr) > 1 )
				{
					...
					foreach($valArr as $k1=>$v1)
					{
						...
						switch ($v)
						{
							...
							case '>':
								if( (!empty(C('page.'.$key)) && C('page.'.$key) > $v1) || intval($key) > $v1)
								{
									$code = $curArr[2][$i];
								}
								break;
							...
							case '=':
								if( (!empty(C('page.'.$key)) && C('page.'.$key) == $v1)  || $key == $v1 )
								{
									$code = $curArr[2][$i];
								}
								break;
		...
		if ( $table != '' )
		{
			switch ( $table )
			{
				...
				case '@upload':
					...
					$wheresql['field'] = 'upload_ext,upload_alt,upload_type,upload_img,upload_simg,upload_size,upload_time,upload_height,upload_width';
					break;
	...
	**/
	...
	private function __CheckWxSub($aid,$wxAcSer,$wxPfSer)
	{
		...
		{
			//获得用户信息
			$userInfo = $wxPfSer->UserGetInfo($wxAcSer->GetOpenid());
			//用户信息获取成功
