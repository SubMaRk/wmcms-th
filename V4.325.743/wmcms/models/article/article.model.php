...
class ArticleModel
{
	...
	function Update($data , $whereArr)
	{
		...
		if( $this->articleSaveType == '2' && isset($data['article_content']) )
		{
			...
			$content = $data['article_content'];
			$data['article_content'] = '';
			$this->SaveTXT($content, $articleData['type_id'], $articleData['article_id']);
		}
	 ...
	 * @param 参数2，必须，文章id
	 */
	function GetArticlePath($tid , $aid)
	{
	 ...
	 * @param 参数2，必须，文章id
	 */
	function GetTXT($tid,$aid)
	{
		$txtPath = $this->GetArticlePath($tid, $aid);
		if( file_exists($txtPath) )
		{
			return file_get_contents($this->GetArticlePath($tid, $aid));
		}
		else
		{
	 ...
	 * @param 参数2，必须，文章id
	 */
	function SaveTXT($fileContent,$tid,$aid)
	{
		$txtPath = $this->GetArticlePath($tid, $aid);
