...
class WeiXin_Platform extends WeiXin{
	 ...
	 * @param 参数2，必须，日志内容
	 */
	function SetLog($type='response',$data)
	{
	...
	function ResponseGetTemp($type)
	{
		$args = func_get_args();
		//基础模版封装
		$template = "<ToUserName><![CDATA[%s]]></ToUserName><FromUserName><![CDATA[%s]]></FromUserName><CreateTime>%s</CreateTime><MsgType><![CDATA[{$type}]]></MsgType>";
		//内容封装
		switch ($type)
		{
			case 'text':
				...
				$template .= "<Content><![CDATA[{$args[1]}]]></Content>";
				break;
			case 'image':
				$template .= "<Image><MediaId><![CDATA[{$args[1]}]]></MediaId></Image>";
				break;
		}
