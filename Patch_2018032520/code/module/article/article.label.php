	...
	static function PublicType($data,$blcode,$level=''){
		...
		foreach ($data as $k => $v)
		{
			...
			$arr2=array(
				...
				$level.'文章分类名字:'.@$nameArr[0]=>@$nameArr[1],
				$level.'文章分类简称'=>$v['type_cname'],
				$level.'文章分类拼音'=>$v['type_pinyin'],
				$level.'文章分类图标'=>C('type_ico',null,$v),
	...
	static function PublicArticle($data,$blcode)
	{
		...
		foreach ($data as $k => $v)
		{
			...
			$arr2=array(
				...
				'文章标题:'.@$nameArr[0]=>@$nameArr[1],
				'文章内容:'.@$contentArr[0]=>@$contentArr[1],
				'文章简介:'.@$infoArr[0]=>@$infoArr[1],
				'文章发布时间:'.@$time[1][0]=>tpl::Time(@$time[1][0], $v['article_addtime']),
			);
	...
	**/
	static function ArticleLabel()
	{
		...
		tpl::rep(array(
			...
			'标题:'.@$nameArr[0]=>@$nameArr[1],
			'内容:'.@$contentArr[0]=>@$contentArr[1],
			'简介:'.@$infoArr[0]=>@$infoArr[1],
			'发布时间:'.@$time[1][0]=>tpl::Time(@$time[1][0], $v['article_addtime']),
		));
