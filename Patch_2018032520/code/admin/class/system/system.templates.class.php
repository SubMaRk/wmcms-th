...
class SystemTemplates
{
	...
	function GetTempType( $module = '' , $val='')
	{
		...
		$tempType['picture'] = array('list'=>'列表页','content'=>'内容页');
		...
		$tempType['zt'] = array('content'=>'内容页');
		$tempType['diy'] = array('content'=>'内容页');
		
		if ( $val != '' && $module != '')
		{
			$tempType = @$tempType[$module][$val];
		}
		else if( $module != '' )
		{
			$tempType = @$tempType[$module];
		}
		return $tempType;
	}
	 ...
	 * @param 参数1，选填，传入的条件
	 */
	function GetTempList( $where = '' )
	{
		//获取列表条件
		$where['table'] = $this->table;
		
		//数据条数
		$data['total'] = wmsql::GetCount($where , 'temp_id');
