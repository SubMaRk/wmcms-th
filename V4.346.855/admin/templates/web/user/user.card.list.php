<div class="bjui-pageHeader">
	<div class="row cl pt-10 pb-10 pl-10">
		<div class="f-l" style="margin-left:10px">
			<a href="index.php?a=yes&c=user.card&t=del" data-toggle="doajaxchecked" data-idname="ids" data-group="ids" data-confirm-msg="คุณต้องการลบรายการที่เล็อกหรือไม่?" class="btn btn-danger size-MINI radius" data-callback="<?php echo $cFun;?>ajaxCallBack"> <i class="fa fa-remove"></i> ลบหลายรายการ</a>
			<a href="index.php?a=yes&c=user.card&t=clear" data-toggle="doajax" data-confirm-msg="คุณต้องการล้างบันทึกทั้งหมดหรือไม่?" class="btn btn-danger size-MINI radius"><i class="fa fa-trash-o"></i> ล้างบันทึก</a>
		</div>
		<div class="f-l">
			<form id="pagerForm" name="<?php echo $cFun;?>Form"  data-toggle="ajaxsearch" data-loadingmask="true" action="index.php?c=user.card.list" method="post">
				...
				<select data-toggle="selectpicker" id="<?php echo $cFun;?>use" name="use" data-width="100">
                	...
                	<option value="0" <?php if($use == 0){echo 'selected=""';}?>>ไม่ใช้</option>
                	<option value="1" <?php if($use == 1){echo 'selected=""';}?>>ใช้อยู่</option>
                </select>
...
<div class="bjui-pageContent">
		<table class="table table-border table-bordered table-hover table-bg table-sort">
			<thead>
				...
				<th width="5%" data-order-field="card_id">ลำดับ</th>
				<th width="6%">ประเภทบัตร</th>
				<th width="6%">สถานะบัตร</th>
				<th width="10%">ช่องทางเผยแพร่</th>
				<th width="14%">หมายเลขบัตร</th>
				<th width="8%">สกุลเงิน</th>
				<th width="8%">รางวัล<?php echo $userConfig['gold2_name'];?></th>
				<th width="14%">เวลาที่เพิ่ม</th>
			...
			if( $cardList )
			{
				...
				foreach ($cardList as $k=>$v)
				{
					...
					$use = str::CheckElse( $v['card_use'] , 1 , '<span style="color:red">ใช้อยู่</span>' , '<span style="color:green">ไม่ใช้</span>');
					echo '<tr class="'.$cur.'">
							...
							<td style="text-align: center;" data-noedit="true">
								<a class="btn btn-danger radius" onclick="'.$cFun.'delAjax('.$v['card_id'].')">ลบ</a>
				            </td>
						</tr>';
					$i++;
				}
...
function <?php echo $cFun;?>delAjax(id)
{
	...
	ajaxOptions['confirmMsg'] = "คุณต้องการลบหมายเหขปัจจุบันทันทีหรือไม่?";
