...
class tpl{
	...
	**/
	...
	function Display( $mark = '')
	{
		...
		else
		{
			...
			$this->rep(array('</title>'=>' - Powered by WMCMS</title>'),'',2);
		}
		...
		{
			$cacheSer->SetSite(GetUrlPath('3' , 1) , $this->RepTempCode(self::$tempCode) , $cacheSer->cacheTime);
		}
	...
	**/
	public static function GetWhere( $str , $arr = '' )
	{
		...
		{
			//设置默认变量
			$page = $pageCount = $limitSql = $orderSql = $whereSql = $groupSql = '';
			//设置标签识别数组
