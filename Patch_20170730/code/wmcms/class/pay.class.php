...
class pay
{
	...
	function Order($data)
	{
		...
		else
		{
			...
			if( $levelData )
			{
				if( $levelData['level_real'] != $money)
				{
					$realMoney = $levelData['level_real'];
				}
			...
			switch ($payType)
			{
				//微信电脑支付，跳转到二维码页面
				case 'wxpay':
					header("Location: ".tpl::Url('user_charge_code',array('sn'=>$orderSn,'code'=>urlencode(Encrypt($result,'E')))));
					break;
