...
class http
{
	...
	function GetUrl($url , $data='')
	{
		...
		if(@$data['cookie_save'] == '1' || @$data['cookie_send'] == '1')
		{
			if(@$data['cookie_name'] != '')
			{
				$cookieFile = WMROOT.'cache/cookie_'.$data['cookie_name'].'.txt';
			}
			else
			{
				$cookeName = md5(basename(__FILE__,".php"));
				$cookieFile = WMROOT.'cache/cookie_'.$cookeName.'.txt';
			}
			//是否保存cookie
			if(@$data['cookie_save'] == '1')
			{
				curl_setopt($ch, CURLOPT_COOKIEJAR,$cookieFile);  
			}
			//是否需要发送cookie
			else if(@$data['cookie_send'] == '1')
			{
