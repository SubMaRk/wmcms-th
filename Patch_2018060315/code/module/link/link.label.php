	...
	static function PublicLink($data,$blcode)
	{
		...
		foreach ($data as $k => $v)
		{
			...
			$arr2=array(
				...
				'友链分类id'=>$v['type_id'],
				'友链分类名字'=>$v['type_name'],
				'友链分类名字:'.@$typeNameArr[0]=>@$typeNameArr[1],
				'友链分类简称'=>$v['type_cname'],
				'友链分类拼音'=>$v['type_pinyin'],
				'友链分类简介'=>$v['type_info'],
				...
				'友链名字:'.@$nameArr[0]=>@$nameArr[1],
				'友链简介:'.@$infoArr[0]=>@$infoArr[1],
				'友链加入时间:'.@$time[1][0]=>tpl::Time(@$time[1][0], $v['link_jointime']),
			);
	...
	**/
	static function LinkLabel()
	{
		...
		$arr2 = array(
			...
			'分类id'=>$v['type_id'],
			'分类名字'=>$v['type_name'],
			'分类名字:'.@$typeNameArr[0]=>@$typeNameArr[1],
			'分类简称'=>$v['type_cname'],
			'分类拼音'=>$v['type_pinyin'],
			'分类简介'=>$v['type_info'],
			...
			'加入时间:'.@$time[1][0]=>tpl::Time(@$time[1][0], $v['link_jointime']),
		);
		//替换自定义字段
