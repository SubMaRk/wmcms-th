...
{
	//session开启关闭状态，1为关闭，2为开启
	//if( session_status() == 1 ) php5.4支持
	if( !isset($_SESSION) )
	{
		session_start();
	}
...
{
	ReturnData( $msg , true , $code , $data , 'jsonp');
}
//返回json格式数据
function ReturnJson( $msg = '' , $code = 200 , $data = '' )
{
	ReturnData( $msg , true , $code , $data ,  'json');
}
//返回xml格式数据
 ...
 * @param 参数5，选填，返回的值的类型
 */
function ReturnData( $msg = '操作成功！' , $ajax = '' , $code = 500 , $data = array() ,  $returnType = 'json')
{
	//如果是wmcms/aciton方法就进入后置事件
	if( C('page.module') == 'action' && $code == 200 && !empty(C('page.action_path')) && !empty(C('page.action_file')) )
	{
...
function GetIp()
{
	...
	else if( isset($_SERVER['SERVER_ADDR']) || getenv("SERVER_ADDR") )
	{
		...
		if( $typeFieldList )
		{
			foreach ($data as $k1=>$v1)
			{
				foreach ($typeFieldList as $key1=>$val1)
				{
					...
					//自定义字段的键名匹配
					//优先级最高，id相等
					if($val1['field_type_id'] == $v1['type_id'] )
					{
						$data[$k1]['t_field'] = $val1['t_field'];
						break;
					}
					//第二优先级，是子级，并且id存在pids里面
					else if($val1['field_type_child'] == '1' && in_array($v1['type_id'], explode(',', $val1['field_type_pids'])) )
					{
						$data[$k1]['t_field'] = $val1['t_field'];
						break;
					}
					//最后优先级，全部分类
					else if($val1['field_type_id'] == '0' )
					{
						$data[$k1]['t_field'] = $val1['t_field'];
						break;
					}
 ...
 * @param 参数5，选填，标签的前缀。
 */
function RepField($arr2 , $arr1 = array() , $value , $type = '1' , $label='')
{
	if( !empty($arr1) )
	{
		...
		if( $v['field'] != '' )
		{
			...
			if( $fieldOption )
			{
				foreach ($fieldOption as $k=>$v)
				{
					...
					{
						$arr3[$label.C('system.label.diy',null,'lang').':'.$v['title']] = GetKey($valueOption,$k);
						$arr3[$label.'field:'.$v['title']] = GetKey($valueOption,$k);
					}
				}
			}
		}
...
function ProcessReturnUser($data = array())
{
	if( $data )
	{
		$data['user_account'] = str::A($data['user_name'], $data['user_psw']);
